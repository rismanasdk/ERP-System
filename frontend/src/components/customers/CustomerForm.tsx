  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input id="customer-code" value={code} onChange={(e) => setCode(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      ...
      <div>
        ...
        <input id="customer-name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
            ...
            <input id="customer-phone" value={phone ?? ''} onChange={(e) => setPhone(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
        </div>
        <div>
            <label htmlFor="customer-email" className="block text-sm font-medium text-slate-700">Email</label>
            <input id="customer-email" value={email ?? ''} onChange={(e) => setEmail(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
        </div>
      </div>

      <div>
          <label htmlFor="customer-address" className="block text-sm font-medium text-slate-700">Address</label>
          <input id="customer-address" value={address ?? ''} onChange={(e) => setAddress(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      </div>

      <div>
          <label htmlFor="customer-taxid" className="block text-sm font-medium text-slate-700">Tax ID</label>
          <input id="customer-taxid" value={taxId ?? ''} onChange={(e) => setTaxId(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      </div>

      <div className="flex items-center gap-4">
          <label htmlFor="customer-active" className="flex items-center gap-2 text-sm">
