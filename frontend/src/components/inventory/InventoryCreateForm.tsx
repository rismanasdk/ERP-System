...
export function InventoryCreateForm({ submitting = false, onSubmit, onCancel }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="product_id" className="block text-sm font-medium text-slate-700">Product ID</label>
        <input id="product_id" value={productId} onChange={(e) => setProductId(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
        {errors.product_id ? <p className="mt-1 text-sm text-red-600">{errors.product_id}</p> : null}
      </div>

      <div>
        <label htmlFor="branch_id" className="block text-sm font-medium text-slate-700">Branch ID</label>
        <input id="branch_id" value={branchId} onChange={(e) => setBranchId(e.target.value)} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      ...
      <div>
        ...
        <input id="quantity" value={quantity} onChange={(e) => setQuantity(e.target.value)} type="number" className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
