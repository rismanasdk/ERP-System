...
export function SaleItemEditor({ value = {}, onChange, onRemove, existingProductIds = [] }: Props) {
  ...
  const loadProducts = useCallback(async (q?: string) => {
    try {
      const res = await productsApi.list({ search: q }, undefined)
      setProducts(res)
    } catch {
      setProducts([])
    }
  ...
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-6 items-end">
      <div className="md:col-span-2">
        ...
        <select value={productId} onChange={(e) => handleProductChange(Number(e.target.value))} onFocus={() => void loadProducts()} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm">
      ...
      <div>
        ...
        <input value={quantity} onChange={(e) => handleQuantityChange(e.target.value)} type="number" min={1} className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      </div>

      <div>
        <label className="block text-sm font-medium text-slate-700">Unit Price</label>
        <input value={unitPrice} onChange={(e) => handleUnitPriceChange(e.target.value)} type="number" min={0} step="0.01" className="mt-1 block w-full rounded-md border-slate-200 shadow-sm" />
      </div>
      ...
      <div className="flex items-center gap-2 md:col-span-2 justify-end">
        {onRemove ? <button onClick={onRemove} className="rounded-md border border-red-300 bg-white px-3 py-1 text-sm text-red-700">Remove</button> : null}
      </div>
    </div>
  )
}
